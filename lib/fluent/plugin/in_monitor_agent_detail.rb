...
module Fluent::Plugin
  class MonitorAgentDetailInput < Input
    Fluent::Plugin.register_input('monitor_agent_detail', self)
    ...
    MONITOR_INFO = {
      ...
      'buffer_total_queued_ratio' => ->(){ throw(:skip) unless instance_variable_defined?(:@buffer) && !@buffer.nil? && @buffer.is_a?(::Fluent::Plugin::Buffer); ((@buffer.stage_size + @buffer.queue_size) / @buffer.total_limit_size.to_f).round(4) },
